...
vec4 envColor = textureLod( envMap, reflectDir,lodLvl); //textureCube
...
for (int i=0; i<maxS*maxS; i++)
{
    ...
    w += tw*bloomRadius; // values less than 1 like 0.85 fake bloom of bright areas
...
envColor = mix( refractedColor, envColor, clamp( vReflectionFactor, 0.0, 1.0 )*1. );
...
envColor.xyz = blendOverlay(envColor.xyz,overlayColor,overlayFactor);
