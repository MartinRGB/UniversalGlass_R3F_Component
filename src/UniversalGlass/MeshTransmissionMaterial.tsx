...
export const MeshTransmissionMaterial = React.forwardRef(
  (
    ...
    const [cubeRenderTarget] = React.useMemo(()=>{
      const rt1 = new THREE.WebGLCubeRenderTarget( 1024, {generateMipmaps: true, minFilter: THREE.LinearMipmapLinearFilter } );
      return [rt1]
    },[])

    const [cubeCamera] = React.useMemo(()=>{
      const cam1 = new THREE.CubeCamera( 1, 100000, cubeRenderTarget );
      return [cam1]
    },[])  
    ...
    useFrame((state) => {
      ...
      if (ref.current.buffer === fboMain.texture && !transmissionSampler) {
        ...
        if (parent) {
          ...
          cubeCamera.position.copy( state.camera.position );
          ...
          //parent.material.buffer = cubeRenderTarget.texture
